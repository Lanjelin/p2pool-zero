...
RUN wget "$p2pool_url/releases/download/$p2pool_tag/p2pool-$p2pool_tag-linux-x64.tar.gz" && \
    wget "$p2pool_url/releases/download/$p2pool_tag/sha256sums.txt.asc" && \
    ...
    expected=$(grep -A2 "Name: p2pool-$p2pool_tag-linux-x64.tar.gz" sha256sums.txt | grep SHA256 | awk '{print $2}') && \
    echo "$expected  p2pool-$p2pool_tag-linux-x64.tar.gz" | sha256sum --check || { echo "Hash mismatch!"; exit 1; } && \
    tar xfz p2pool-$p2pool_tag-linux-x64.tar.gz --strip-components=1
